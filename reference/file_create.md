...
truncated), matching
[`fs::file_create()`](https://fs.r-lib.org/reference/create.html)'s
local behavior; a missing one is created empty via an upload. Unlike the
local version, an existing remote file's modification time is not
bumped.

## Usage

``` r
file_create(path, con = NULL, ...)
```
