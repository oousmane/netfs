...
On a backend without server-side
[`file_copy()`](https://oousmane.github.io/netfs/reference/file_copy.md)
(currently FTP), each file is copied through a local staging download
and upload instead, so the copy still completes - just more slowly than
a native copy.

## Usage

``` r
dir_copy(path, new_path, con = NULL, overwrite = FALSE)
```
...
- new_path:
  ...
  Destination on the same filesystem and connection as `path`.
...
The destination path, invisibly, as an `fs_path`.
