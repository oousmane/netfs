...
- path:
  ...
  One or more local or remote paths.
...
Remotely, a tibble with one row per `path` and `path`, `type`, `size`,
and `modification_time` columns, typed like their
[`fs::file_info()`](https://fs.r-lib.org/reference/file_info.html)
