...
- fun:
  ...
  A function to apply to each path.
...
- fail:
  ...
  Passed to
  [`fs::dir_map()`](https://fs.r-lib.org/reference/dir_ls.html) locally.
  Not configurable remotely: a listing error always fails the call.

## Value

Locally, the result of
[`fs::dir_map()`](https://fs.r-lib.org/reference/dir_ls.html). Remotely,
a list of `fun`'s results, one per entry.
